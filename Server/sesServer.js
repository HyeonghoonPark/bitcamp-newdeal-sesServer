...
Router.get('/sendEmail2', (req, res) => {
    
 console.log('router');
    
    emaildao.select(req.query, (err,result) => {
        ...
        if (err){
            ...
            <h1>BCM 계정</h1>
            <p>안녕하세요. BCM입니다.<br>
                BCM 서비스 이용을 위해 BCM계정 비밀번호 찾기 인증을 요청하셨습니다. <br>
                BCM 임시 비밀번호 : ${result[0].TPWD} <br>
                             임시 비밀번호로 로그인 후 비밀번호 변경 서비스를 이용하여 비밀번호를 변경해주세요.<br>
                             해당 임시 비밀번호는 24시간 뒤 사용이 불가능 합니다.
            </body>
            </html>`;
...
Router.get('/issueTPWD', (req, res) => {
    
    console.log('router');
    
    emaildao.tpwdUpdate(req.query, (err,result) => {
        ...
        res.writeHead(302, {"Location":`http://localhost:8000/sesServer/sendEmail2?sendmail=${req.query.sendmail}`});
        
        res.end('임시비밀번호 성공입니다!'); 
